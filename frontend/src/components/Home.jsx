...
function Home() {
  ...
  const logoutBtn = async () => {
    try {
      await dispatch(logoutUser({ access: accessToken, refresh: refreshToken }))
      navigate('/login')
    } catch (error) {
      navigate('/login')
    }
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
              {!localStorage.getItem('access')
                ...
                : <button
                  ...
                  className="bg-red-600 text-white px-4   py-2 !rounded-full border-none text-sm 
w-auto transition-all duration-400 ease-in-out 
hover:bg-purple-700 hover:scale-105 hover:shadow-5xl 
active:scale-95 active:shadow-xl"        >
          ...
          <div className="flex justify-center px-4 mb-6">
            ...
            <input
              type="search"
              placeholder={`🔍 Search for ${clickedState}...`}
              value={searchVal}
              onChange={(e) => {
